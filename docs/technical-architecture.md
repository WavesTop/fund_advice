...
详细设计见[界面展示与交互开发文档（待审查草案）](ui-interaction-design.md)，已结合 §3.2–§3.6 补充页面结构、业务展示、跨页交互、异常状态及验收场景。以下保留整体架构中的设计要点。
...
第 3 节作为整体设计索引，各主题的详细设计分别维护。当前 §3.1 已形成[界面展示与交互开发文档](ui-interaction-design.md)，等待审查；其余主题后续继续补充数据字段、接口约定、计算公式、策略参数、任务恢复机制和验收用例。界面文档中依赖这些主题的业务规则，在对应设计确认后同步校对。
...
前端 `features` 按“基金与板块、我的持仓、建议与决策、收益与复盘、设置”组织；后端 `modules` 按“基金与板块、持仓账本与交易规则、投资分析、建议与决策、模拟与收益、复盘与策略改良”划分业务职责。
