...
详细设计见[基金数据与存储开发文档](fund-data-storage-design.md)，已结合现有界面及 §3.3–§3.6 的依赖，明确数据来源验证、身份与版本模型、存储和精度、采集更新、API、开发批次及验收场景。当前为开发设计稿，真实数据接入尚未实现。以下保留整体架构中的设计要点。
...
详细设计见[投资分析与建议开发方案](investment-analysis-design.md)，实施拆分见[分阶段开发与复查计划](investment-analysis-plan.md)。两份文档已覆盖研究逻辑、数据依赖、证据融合、具体基金匹配、个人操作、AI、界面和验收；当前已在“板块机会”实现有限行业事实、估值观察与真实走势结合的三周期筛查（方案 §1.5），完整推荐链路仍为开发设计，投资效果尚未验证。
