...
- **走势强度及排序**：短／中／长期走势分别描述过去 20／60／120 个交易日，按同周期价格涨跌幅排名，支持从强到弱及从弱到强。排名在行业池和参考指数池内分别确定价格比较日，仅比较最新日期与该池比较日相同、观察起点与同组多数样本相同的有效样本；成交额榜单日单列，不替代价格比较日，防止缺行导致实际区间不同；失效样本两种排序均置末。热度池和原有指数各自计算名次，不能混称同一个榜。价格变化按展示精度并列，名次为竞争排名（如 1、1、3），分位按并列平均位置计算、0 最弱／100 最强、全体相同为 50，单一样本不输出分位。该数值不是胜率或投资综合分。搜索、筛选和分页不改变原始样本排名，代码稳定打破展示顺序中的平局。前 25% 准入使用未四舍五入的位置分位：`4×低于本值数量 + 2×(并列数量−1) ≥ 3×(样本数−1)` 且样本数大于 1；显示为 75.0 但实际不足 75 的对象不入选。保留原收益两位小数并列规则，不把展示分位改成新评分。
...
- **时间与缺失**：只使用计算日及以前行情；最新行情超过 10 个自然日、窗口内间隔超过 14 个自然日、无效价格或样本不足时暂停价格判断。行业最近公布数据超过 45 个自然日时暂停经营状态；估值观察或已知业务日期超过 10 个自然日只供核对旧资料；重新下载旧业务日期的值不能恢复“当前可用”。公开／取得时间晚于计算时点的证据不能使用。上述日数是初始保守规则，尚未接入完整交易日历；源端当日行情仍可能修订。
- **证据身份与版本**：配置中的每条证据必须显式给出适用 `subject.source_id` 和 `subject.universe_type`，并与所属代码共同匹配研究对象；缺少来源、同代码异源或跨行业／指数池不复用。现有三份背景只补充适用身份，不增加数字或新来源。当前方法 `evidence-screen-v3`、强度准入 `common-window-quartile-v2`；价格状态仍为 `price-state-v2`。
- **运行与验证边界**：重新评估只读本地资料，不调用 AI、不外部采集、不执行交易。“获取最新行业数据”是独立 POST，复用已存在的行业采集器后再读取评估；不是经营／估值自动更新。页面披露方法、评估时间、来源和缺项。当前没有足量历史非价格快照、成分穿透或样本外证据，不宣称预测准确率；计算及界面测试通过只说明规则正确执行。完整链路仍按 A0—A6 实施，此轻量入口不替代对应验收。
...
**展示入口。** 基金库继续查询真实目录，按实际容器每页 6／8 张卡片，当前唯一已核验关联可直达真实板块详情；搜索词、页码与每页数量写入 URL；历史词、示例搜索与浏览器前进后退走同一请求流程。切换查询取消旧请求，响应不得覆盖新查询。详情固定真实代码与份额身份，区分行情业务日期和入库日期；基金披露、行业穿透或费用不足时保留缺项，不能展示模拟持仓。板块默认使用三周期摘要表，支持切换完整证据、完整成分查看、返回恢复筛选和按来源／代码进入研究。
...
MW1 的回归验证已覆盖当前规则的身份、时间、数值、相邻报告期与分位边界；它不是上表新数据的真实接入，更不是收益有效性证明。现有 20／60／120 窗口、±1% 方向、均线、回撤和同日同起点规则保留，不以一轮代码修复改写未来预测口径。实际运行与未运行的检查只在计划 MW1 和界面测试报告当前小节记录。
