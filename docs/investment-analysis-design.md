...
#### 1.11.1 历史可展示不等于当前可排名

价格方法升级为 `price-state-v5-historical-display`。当最新完整交易日尚未采集，当前周期的 stale／insufficient 和排名准入保持原规则；按真实已存截止日再次严格核验连续窗口，仅合格窗口输出 `historical` 的涨跌、均线偏离、回撤、起止日期。该对象不包含排名或投资资格，不能将 stale 主字段直接当成历史数值使用；更新失败时同样不恢复当前排名。

页面及研究文字可以引用带明确截止日的历史指标。经营／估值继续用各自报告期、公告时间和业务日判断，不因历史行情可展示就放开推荐；受支持证券样本须和全板块覆盖并列。新增的资料展示不证明完整真实采集或预测能力。
