...
**展示入口。** 基金库继续查询真实目录，搜索词与页码写入 URL；历史词、示例搜索与浏览器前进后退走同一请求流程。切换查询取消旧请求，响应不得覆盖新查询。详情固定真实代码与份额身份，区分行情业务日期和入库日期；基金披露、行业穿透或费用不足时保留缺项，不能展示模拟持仓。板块默认使用三周期摘要表，支持切换完整证据、完整成分查看、返回恢复筛选和按来源／代码进入研究。
...
**当前边界。** 本批不新增数据库迁移，不改变采集算法，不调用 AI，不产生交易或金额。`recommendation_status=not_evaluated`、`operation_status=unavailable` 保持明确，不由缺项数量或涨幅自动升级。结果仍是当前投影，不是不可变输入快照；正式推荐、历史复算和效果验证继续按 A0—A6 的前置关系实施。行情失败保留有效经营反证，不能把风险抹成中性。
