...
输入只取指定 snapshot，固定对象、原始版本、质量排除与日历，规则版本为 `snapshot-research-v1`。输出写入不可变 `analysis_run`，关联输入清单哈希、实现代码哈希及输出哈希。重复同快照／同实现运行幂等；当前代码与旧运行实现哈希不同，重放返回冲突，仍可读取原始结果，不能拿新算法声称旧版复算通过。资料修订另建快照和运行，不覆盖旧报告；当前没有自动修订影响图和失效调度。
