...
当前已有 FastAPI／SQLite、真实基金目录、部分净值／场内日行情及关联指数的最小切片，建议页默认改为当前真实研究，原交互演示通过显式入口保留；完整正式推荐仍未完成。`backend/storage/timeseries.py` 导入时替换当前序列，`related_market.py` 保存当前关联与序列；它们尚无本方案所需的不可变事实版本、完整身份关系和历史输入快照。已有真实图表不等于可以复现历史研究。
...
**当前结论：可以作为来源可核对的历史走势和研究线索工作台，不能作为经过投资效果验证的板块择时或基金买入系统。** 本轮修复数据质量／执行顺序，不新增任意权重、胜率或买入阈值；`formal_recommendation_ready=false`、`recommendation_status=not_evaluated`、`operation_status=unavailable` 不变。
...
| 行情与信号 | 20／60／120 回看、±1%、均线、−10% 回撤及前 25% 都是描述性初始规则；历史强势不等于未来超额收益。 | 本轮先修正盘中、整榜失败、舍入并列及内部日期不齐。交易日历、总收益、基准相对收益、波动及流动性仍须真实接入并独立验证；不机械叠加重复价格指标计票。 |
| 重新评估输入 | 行业采集不更新人工非价格配置及参考指数，SQLite 仍为可变当前投影；单进程锁覆盖采集后计算，不覆盖其他进程／CLI。 | 逐来源标记业务日、公开日、取得日和失败状态；按 A1.1／D4 固定不可变输入及方法版本，验证跨模块一致性后才宣称历史可复算。 |
| 基金可投资性 | 已核验指数链接只是关系，不是基金择优；没有完整跟踪差异、持有成本、交易约束、主动管理归因。 | 被动基金核对跟踪表现、费用、规模／成交与折溢价；主动基金核对经理任期、风格和披露滞后；联接穿透、份额类别及个人限制分别处理。 |
