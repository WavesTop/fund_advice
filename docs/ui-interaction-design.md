...
- 更新日期：2026-09-16
- 状态：历史 §3.1 演示界面已实现，基金目录与走势已有真实接入；当前优先补齐展示与真实研究，实施及验证状态见投资分析计划 L2。
- 对应章节：[整体技术架构 §3.1](technical-architecture.md#31-界面展示与交互)。
- 需求依据：[产品目标与用户流程](product-goals-and-user-flow.md)，以及整体技术架构 §3.2–§3.6。
- 历史界面验收：完整页面与演示交互的历史范围见[界面测试报告](ui-test-report.md)；本批真实展示与研究仍须完成 L2 的接口、组件和浏览器验收。

### 当前实现边界

基金库和已采集基金详情展示本地真实目录、净值或交易日线；“板块机会”已有真实三周期行情及有限行业证据。持仓、交易、复盘、策略和旧建议交互仍为演示，不能再笼统描述为所有页面均是虚构数据。历史 §3.1 界面测试报告仅证明当时的演示范围。

当前优先范围是基金／板块展示和真实研究／对应推荐，步骤与状态统一见 [投资分析计划 §1.4](investment-analysis-plan.md#14-轻量入口)。保留浅色面板、蓝色主操作与三周期识别，不重做导航，不在当前批次接入真实账本或 AI。

| 页面 | 本批修改要求 | 不得误导的边界 |
| --- | --- | --- |
| 基金库 | 保留搜索后显示结果的交互；名称／六位代码搜索、URL 页码和历史词统一；直接进入真实详情或关联研究；请求取消与身份校验。 | 可搜索不代表资料完整，更不代表推荐；无结果不回退示例。 |
| 基金详情 | 明确名称、代码与份额身份；分别显示真实 K 线／净值、实际区间、行情截止日、入库日和来源；研究入口保留真实代码。 | 未接入的持仓披露、板块穿透、费用与筛选保持缺项；不画伪走势。 |
| 板块机会 | 默认三周期摘要表，可切完整证据；基金关联、实际历史排名、反证、资料日期分开；完整成分可查看；返回恢复搜索／范围／排序／页码。 | 金融涨跌用正负号及方向色；投资状态用单独文字徽标；历史涨幅不是买入理由。 |
| 投资分析与建议 | `/advice` 默认真实研究；上方对象与资料状态、三周期观察、对应基金关联、全部研究对象；证据与原始指标可下钻。 | 基金映射不足明确展示；正式推荐与操作保持未评估，不由演示或未知数据填充。 |
| 演示建议与记录 | `mode=demo` 显式进入，旧决策链接兼容且继续标识演示。 | 真实研究不写入演示决定；演示记录不作为正式建议历史。 |

三周期在宽屏同页对照，窄屏纵向保留全部周期；板块宽表在自身容器滚动。空白、首次加载失败、刷新失败保留旧结果、资料不足／过期、无基金映射、无投资结论使用不同文案。所有失败可重试；更新时不得把入库或读取时间当作行情日期。首批界面不增加综合评分、胜率、默认金额或交易按钮。

视觉目标与状态设计不代表已经通过实际浏览器验收。完整前端构建、组件测试、真实数据联调及 1440／768／390px 复查分别执行，结果只在分阶段计划记录。
...
未提交搜索时，在搜索框下方展示实际浏览过的基金卡片；没有浏览记录显示明确空态，不用全库基金或搜索词猜测补齐。提交搜索后在同一区域展示搜索结果，不再使用每页 20 行表格。容器宽度至少 1,180 px 时 4 列、每页 8 张；840–1,179 px 为 3 列、每页 6 张；560–839 px 为 2 列、每页 6 张；更窄为 1 列、每页 6 张，末页按剩余数量展示。以实际容器测量为准，不用屏幕分辨率猜测；宽度跨越 6／8 切换点时回到第一页，避免偏移重复或漏项。

搜索右侧“清空”同时清除输入、已提交关键词和页码，恢复浏览记录第一页，不删除浏览记录。只有详情请求成功、代码与请求一致且名称有效时写入浏览记录；失败、404、取消和单纯搜索不记入。浏览代码以 `fundAdvicer.realFundVisits.v1` 本地保存，最近优先、去重、上限 60 个；卡片元数据和关联每次批量读取本地数据库，跨标签页变更同步，损坏或禁止存储不阻止浏览。以前未记录的访问不可恢复。

搜索词、页码和每页数量写入 URL；最近 8 个成功请求的搜索词仍单独在浏览器本地去重保存，可复用或清除。翻页、历史复用、前进后退与重试共用请求逻辑，旧响应不能覆盖新查询；同一查询重新读取失败可保留上次成功卡片并提示旧结果。错页响应不展示，无结果保留搜索词，超出有效页可返回第一页。

卡片展示真实基金名称／代码／份额 ID／类型／目录来源及关联状态。名称进入基金详情，唯一且当前 `linked` 的关系提供真实板块／指数直达链接及核验来源、时间；`missing`、`withheld`、歧义和旧关系只显示原因，不按名称推断。卡片没有取得净值或持仓时不填示例值。类型／行业筛选、按代码补录确认及完整状态任务页仍是前文设计目标，不能因为本卡片完成而视为已实现。

“更新基金目录”发送真实 POST，成功或失败后均重新读取当前查询。它只更新目录身份，不表示已采集基金净值／板块行情。具体接口、超时及并发范围见数据设计 §1.4。
...
原始净值、累计净值、价格和指数仍按各自原值绘制，不输出总收益或超额收益。关联已核验但行情为空时有单独说明；历史／待核验关系保留可读原因，不默认绘图或生成候选。分项刷新成功后立即展示成功部分，保留失败原因和各自资料日期；全部刷新失败但关系核验状态已改变时，消费错误响应中的同基金当前详情，保留基金行情并停止绘制未通过核验的关联指数。

板块完整证据按需加载来源精确的真实日线；跨源参考明确标识，不将同名视为编制方法等价。优势列表分别显示价格比较日与成交额榜单日。MW1 将日线缓存身份扩展到数据日期、入库时间、失败状态和实际行情来源；已展开图表在这些字段变化后重新请求，加载中不继续展示旧版本图表。
...
#### 4.3.1 当前真实板块详情与更新契约（MW1）

真实详情路由为 `/sectors/{code}?source_id=...&universe_type=hot_board|tracked_index`；基金卡片和完整证据标题使用此入口。接口必须精确核对三个身份字段；参数不足、对象不存在、响应错源或读取失败均展示边界状态，不回退第一条板块或演示数据。原未携带来源的演示入口继续隔离保留。`from` 恢复来源列表查询，`fromFund` 只保留导航上下文；详情读取时关系已失效，明确提示未通过当前核验，不能用 URL 参数证明关联。

详情复用现有三周期证据卡片、来源、缺项及按需真实日线。无入库日期保留 `null` 并显示暂无，不编造更新时间。行业详情及板块机会只保留“重新评估”：POST 先采集公开行业数据，再计算并返回评估；直接展示本次响应，不用另一次 GET 冒充本轮结果。初次进入页面仍为本地 GET。局部失败显示“部分更新完成”，全部失败、超时、缺少评估或采集成功但计算失败都不能显示评估完成；失败后重读本地状态用于核对。整榜失败时旧行情不参与当次排名。按钮执行中禁用并防重复，HTTP 404 提示检查旧后端和代理，不静默改为本地重算。基金详情原有分项刷新行为不重复建设。

资料覆盖按行业／板块池和参考指数池分别显示总数、三周期可比数量、有效经营依据、估值原始观察及有明确业务日期且未过期的数量。估值存在不等于估值方法已核验，更不等于投资便宜；经营／估值人工快照及参考指数未由行业按钮更新时明确提示。真实源、锁定环境前端及 1920／1440／768／390 px 的验收状态见投资分析计划 MW1，不借用历史演示验收结果。

#### 4.3.2 当前成分证据展示

板块完整卡片、真实详情与研究详情显示“资料采集与覆盖”：报告期／同比基期、收入／归母利润／现金流、完整分母及未覆盖明细、业务日估值、正／非正 PE、历史观察次数、来源及行业专属待采集项。区分未采集、采集中、失败、部分可用、过期及成分变化；不能把失败重新命名为“没有机会”。旧后台未返回新契约时显示版本缺口，不填入演示资料。

研究页复用原“重新评估”POST；使用本次返回的评价，成功后不另发 GET，失败重读本地已提交状态。按周期显示后端真实 label，不再固定“研究观察”；一个对象可短期上涨、中期业绩待确认、长期估值历史不足。批量研究概览仍按对象分别展示，不把标签变成投资排名。先显示判断及反证，原始报表覆盖放在三周期下方，避免三列重复同一张大表。组件与完整浏览器验收尚未通过，状态统一见投资分析计划 §10。

#### 4.3.3 历史截止日、缓存与采集任务

板块机会、三周期对照表、研究详情使用相同历史指标展示规则：只有后端明确返回且通过数值／日期检查的 historical 可展示；必须标记历史截止日和“当前不可比较”，不把旧排名恢复。缺少最新日不再让已核验的全部历史指标变成横线；内部缺日仍显示不可用。

同一浏览器页面进程保存上次有效结果及按目标共享的采集 Promise。进入页面仅 GET 读取；路由离开不取消已经提交的 POST，返回后订阅该任务，不能再创建相同目标请求。较早GET不得覆盖已完成的新POST，失败保留旧结果与错误。整页重载后的任务恢复仍由后端互斥和只读任务接口协助诊断，不声称完整自动恢复。经营覆盖显示全成分、已接入及未接入数量，不能把局部样本当作全行业。

页面显示采集任务状态、阶段和数据库标识；字段缺失、任务未开始、来源失败与研究资料缺项分别保留。没有开启页面就自动请求所有外部来源，也没有增加重复的“获取最新行业数据”按钮。
