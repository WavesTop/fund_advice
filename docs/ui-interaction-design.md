...
- 更新日期：2026-09-13
- 状态：已作为 §3.1 开发依据，前端界面验收版已实现；真实接口联调尚未开始。
- 对应章节：[整体技术架构 §3.1](technical-architecture.md#31-界面展示与交互)。
- 需求依据：[产品目标与用户流程](product-goals-and-user-flow.md)，以及整体技术架构 §3.2–§3.6。
- 本轮交付：完整页面与演示交互，验收范围及测试结果见[界面测试报告](ui-test-report.md)。

### 当前实现边界

本轮实现导航、搜索筛选、详情图表、三周期对照、分步录入、决定／交易关联、更正留痕、复盘、策略确认和设置交互。录入操作使用可聚焦的宽对话框承载分步表单，替代初稿建议的独立表单页，保留原页上下文。界面始终标识演示状态；金额输入按字符串保存，不在前端核算真实资产、费用或收益。

基金库顶部直接提供两只场内 ETF 的模拟 K 线预览，与详情共用蜡烛图、MA5／MA10／MA20、成交量、日期范围及缩放交互。模拟开高低收独立生成，不由场外基金净值补造；仅有净值的基金继续显示净值走势。

基金身份、行情、披露、建议和比较结果均为固定虚构样例，不是当前市场信息；未知身份不会自动补造。个人填写项保存在当前浏览器的演示草稿中。导入／导出仅覆盖此类 JSON 演示记录；真实备份、AI 调用、资料获取及后台执行不可用，按钮明确说明待接入。任务入队不自动变成成功。

下文保留完整产品设计目标；涉及 §3.2–§3.6 的业务能力仍需实现对应服务并联调，不能据本轮页面测试认定真实账本、投资分析、收益公式或任务恢复已经通过验收。
