...
实际入口保留“未搜索只展示目录总数、搜索案例与最近搜索”；提交后在搜索框下方显示卡片列表，不再使用每页 20 行表格。容器宽度至少 1,180 px 时 4 列、每页 8 张；840–1,179 px 为 3 列、每页 6 张；560–839 px 为 2 列、每页 6 张；更窄为 1 列、每页 6 张，末页按剩余数量展示。以实际容器测量为准，不用屏幕分辨率猜测；宽度跨越 6／8 切换点时回到第一页，避免偏移重复或漏项。

搜索词、页码和每页数量写入 URL；最近 8 个成功请求的搜索词在浏览器本地去重保存，可复用或清除。翻页、历史复用、前进后退与重试共用请求逻辑，旧响应不能覆盖新查询；同一查询重新读取失败可保留上次成功卡片并提示旧结果。错页响应不展示，无结果保留搜索词，超出有效页可返回第一页。
...
详情复用现有三周期证据卡片、来源、缺项及按需真实日线。无入库日期保留 `null` 并显示暂无，不编造更新时间。行业详情及板块机会的“获取最新行业数据”访问公开源并回写，之后重读研究；“重新评估”仍仅本地计算。局部失败显示“部分更新完成”，全部失败或超时不能显示完成；基金详情原有分项刷新行为不重复建设。
