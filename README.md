...
在已合并的浏览／重新评估功能上，新增官方2025—2026日历、有限重试／采集审计、版本事实和固定快照、同基准被动基金比较、历史诊断与前瞻登记。**这不是完整投资推荐系统，也没有证明策略有效。** 当前成分报表／同日估值采集与页面接入见下节；真实来源覆盖仍待验收。 详细规则见[数据设计 §1.5](docs/fund-data-storage-design.md#15-时点研究数据基础当前实现基于-mainfdbb146)及[投资分析设计 §1.10](docs/investment-analysis-design.md#110-固定研究被动基金比较及验证工具当前后台切片)，状态和本轮测试见[投资分析计划 §10](docs/investment-analysis-plan.md#10-进度维护与执行入口)。

行业“重新评估”仍只有一个按钮。服务器采集后返回旧页面使用的 evaluation，并另存固定研究，响应的 research 包含快照／运行引用和状态。固定研究的错误单独可查，不把采集成功当正式研究或推荐成功。原人工经营／估值配置及参考指数不由行业采集自动更新。当前没有新的研究快照、基金比较或验证管理页面，新增能力通过 API／CLI 使用；成分证据已接入原研究／板块页，但前端构建与浏览器尚未验收，不因此将 A5 标为通过。
...
基于 `8483c4d023ed09564a5ce03d2632704835086d66` 的本次增量包含迁移 `013_sector_fundamentals.sql`。**应用新代码并启动前，先停止旧服务，使用旧版本的一致备份功能备份 SQLite；不要只复制 WAL 模式数据库的主文件。** 保留原迁移，不重复应用上一轮研究管线补丁。

板块机会、真实行业详情及研究页复用 `POST /api/sectors/refresh`。行业目录／日线／成分采集后，批量读取当前已结束的两期及去年同期收入、归母利润和经营现金流，并请求最新完整交易日的成分 PE；同一报表请求在多个板块间复用。每项列出原始响应、报告期、公告日期、系统观察时间、覆盖分母和失败原因。旧人工背景和参考指数不在本轮批量采集范围。

三周期分别评价：短期显示已核验价格方向和回撤；中期核对同一成分样本收入／归母利润；长期核对持续经营、现金流与同口径估值。页面不再固定显示“研究观察”，不把经营缺项等同于没有短期走势；也不把价格上涨当作盈利或低估的证明。金融行业不套用一般实业现金流正负规则。正式推荐与个人操作仍未开放。

估值只显示带业务日期的正 PE 成分中位数（**非官方板块 PE**），披露非正值、缺失和样本变化。历史只积累系统实际观察的同一成分及正 PE 样本，同一天重复采集不会增加历史数量；行业专属供需／催化仍是明确待补项。详见数据设计 §1.6、投资分析设计 §1.11，唯一当前检查结果在投资分析计划 §10。

### 板块数据读取与故障修复

再次进入板块机会会先展示同一浏览器页面进程已取得的结果，再用 GET 读取数据库现状；不会自动发起联网采集。手动“重新评估”的同目标 POST 在切换路由后仍由共享请求管理，返回页面复用任务及最终结果。浏览器整页重载不保留内存请求，后端已有数据库互斥继续拒绝重复采集；这不是自动调度或完整任务恢复队列。

缺少最新交易日时，只把当前排名标为不可用。已通过实际截止日连续性校验的历史指标放在 `period.historical`，页面注明截止日；内部缺日、无效价格仍不能生成指标。成分身份与附属市值分别校验：异常市值保存为 null，返回缺失数量，不把整个有效成分列表丢弃。收入／利润／现金流分别披露全板块及受支持范围的覆盖，受支持范围完整不代表全板块完整。

`GET /api/sectors/collection-status` 可读取最近任务、最后阶段、请求次数和不含真实路径的数据库标识。`GET /api/sectors/opportunities` 也返回 `collection_status`；成功 POST 返回其自身任务的终态，失败任务保留最后执行阶段。可据此区分尚未执行、行情阶段失败、报表阶段失败及不同数据库问题。基础修复不自动补出未采集的经营资料、未支持市场或历史估值。

本轮基于 `d0bf777b51c05fab82069460802061e3ff232721`，不增加或改写数据库迁移、依赖锁文件；已有数据库仍使用迁移013。应用补丁后重启前后端，显式执行“重新评估”才会获取新数据。测试及尚未验收的真实源／浏览器状态统一见投资分析计划 §10。
...
| 板块机会／真实行业详情／研究页“重新评估” | `POST /api/sectors/refresh` 联网采集可核验行业目录、日线、成分及成分经营／同日估值，成功或部分成功后在同一次请求内计算并返回评估；前端直接显示该响应，不另发成功后的 GET。 | 不更新经营／估值人工快照，也不批量更新已有参考指数；参考指数仍沿关联基金详情更新。不调用 AI、不执行交易。 |
...
该命令会访问公开行情来源，保存真实覆盖与失败原因；榜单失败时保留旧榜并显示失败标识。行业快照与估值观察保存在 `config/sector-industry-evidence.json`、`config/sector-valuation-evidence.json`，含发布日期、核验时间和来源限制；它们不会自动更新。该行业命令只采集行情与成分；完整报表与估值采集请使用页面“重新评估”。未补齐的维度列具体缺口，不抹掉其他周期已有证据。方法与边界见[开发方案 §1.5](docs/investment-analysis-design.md#15-轻量板块机会的当前实现)。
